...
const Home = (function () {
  const Home = BaseView.extend(function (el, template, data) {
    ...
    if (
      !this.url.params ||
      this.app.homeSections.map((d) => d.id).indexOf(this.url.params.section) ==
        -1
    ) {
  ...
  Home.prototype.onRender = function onRender() {
    ...
    for (let section of this.data.sections) {
      ...
      section.view = new section.view(
        this.el.querySelector(`#${section.id}`),
        section.template,
        {
          app: this.app,
          name: section.id,
  ...
  Home.prototype.onRemove = function onRemove() {
    document.body
      .getElementsByTagName("footer")[0]
      .classList.remove("scroll-section");
  };
    ...
    return Promise.all(
      sections.map(function (section) {
        return new Promise(function (done, error) {
          fetch(
            _env.publicURL + `templates/home-sections/${section.id}.html`
          ).then(function (res) {
  ...
  Home.prototype.lazyLoadSectionBackground = function lazyLoadSectionBackground(
    section
  ) {
    this.data.sections[section] &&
      this.data.sections[section].view.el.classList.remove("lazy");
  };
