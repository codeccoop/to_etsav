...
const Documents = (function () {
    ...
    this.load(_env.apiURL + "documents.json").then(function (response) {
      ...
      data.documents
        .find(function (collection) {
          return collection.name === "Idees";
        })
        .files.find(function (doc) {
          return doc.id === "eternari";
        }).file =
        "eternari." + location.hash.match(/lng=(en|es|ca)/)[1] + ".zip";
      self.data = data;
    });
  };
  ...
  Documents.prototype.onClickMain = function (ev) {
    ...
    ev.currentTarget.parentElement.addEventListener(
      "mouseleave",
      this.onMouseLeave
    );
  };
  ...
  Documents.prototype.onMouseLeave = function (ev) {
    ...
    ev.currentTarget.getElementsByClassName("doc-col-main")[0].style.display =
      "block";
